...
class Sheets(commands.Cog):
    ...
    def __init__(self, client):
        ...
        self.buyorderURL = "https://docs.google.com/spreadsheets/d/.../export?format=csv&id=..."
        self.sellorderURL = "https://docs.google.com/spreadsheets/d/.../export?format=csv&id=..."
    ...
    async def updateprices(self, ctx):
        ...
        await ctx.send(
            'Updating weekly average prices. This might take awhile.')
        ...
        for (i, itemID) in enumerate(itemIDs):
            ...
            for date in dates:
                ...
                try:
                    fullURL = self.apiURL + itemID[
                        0] + '?date=' + date + '&locations=Thetford,Martlock,Lymhurst,Bridgewatch,FortSterling'
    ...
    def sheetsFetch(self, option):
        ...
        if option == 'buyorder':
            ...
            title = 'Pangolin Guild Market            :moneybag: **BUY ORDERS** :moneybag:'
            quantity = 'Remaining'

        elif option == 'sellorder':
            URL = self.sellorderURL
            title = 'Pangolin Guild Market            :moneybag: **SELL ORDERS** :moneybag:'
            quantity = 'Quantity'
        ...
        em = discord.Embed(title=title,
                           description=self.marketURL,
                           colour=color)

        # Tier and enchantment names to remove
        tierNames = [
            "Beginner's", "Novice's", "Journeyman's", "Adept's", "Expert's",
            "Master's", "Grandmaster's", "Elder's", "Beginner", "Novice",
            "Journeyman", "Adept", "Expert", "Master", "Grandmaster", "Elder",
            "Stonestream", "Rushwater", "Thunderfall"
        ]
        ...
        for (i, row) in df.iterrows():
            ...
            try:
                ...
                if df['Quality'][i] in [
                        "Good", "Outstanding", "Excellent", "Masterpiece"
                ]:
