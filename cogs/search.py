...
class Search(commands.Cog):
    ...
    def __init__(self, client):
        ...
        self.guildURL = 'https://gameinfo.albiononline.com/api/gameinfo/guilds/'  # + ID + /members
        self.playerURL = 'https://gameinfo.albiononline.com/api/gameinfo/players/'  # + ID
        self.searchURL = 'https://gameinfo.albiononline.com/api/gameinfo/search?q='  # + name
    ...
    async def search(self, ctx, option, *, name):
        ...
        try:
            ...
            if option.lower() == 'player' or option.lower() == 'players':
                ...
                pvp = data['KillFame']
                gathering = data['LifetimeStatistics']['Gathering']['All'][
                    'Total']
                fiber = data['LifetimeStatistics']['Gathering']['Fiber'][
                    'Total']
                ...
                timestamp2 = DT.datetime.strptime(timestamp,
                                                  "%Y-%m-%dT%H:%M:%S.%fZ")
                ...
                # Second row
                em.add_field(
                    name='Total Fame', value=f'{totalFame:,}', inline=False)
                ...
                # Fourth row
                em.add_field(
                    name='Gathering Fame', value=f'{gathering:,}', inline=True)
                em.add_field(
                    name='Crafting Fame', value=f'{crafting:,}', inline=True)
                em.add_field(name='\u200b', value='\u200b', inline=True)
            ...
            # Guild
                ...
                foundedOn = DT.datetime.strptime(foundedOn,
                                                 "%Y-%m-%dT%H:%M:%S.%fZ")
                ...
                for (i, member) in enumerate(data):
                    ...
                    gatheringFame = member['LifetimeStatistics']['Gathering'][
                        'All']['Total']
                    craftingFame = member['LifetimeStatistics']['Crafting'][
                        'Total']

                    # Each member's fame
                    fames.append(pvpFame + pveFame + gatheringFame +
                                 craftingFame)
                ...
                # Create Discord embed
                em = discord.Embed(
                    title=f':crossed_swords:**{guild}**:crossed_swords:')
                ...
                # Second row
                em.add_field(
                    name='Total Fame', value=f'{totalFame:,}', inline=True)
                em.add_field(name='Members', value=memberCount, inline=True)
                em.add_field(name='\u200b', value='\u200b', inline=True)

                # Third row
                em.add_field(
                    name='PvE Fame', value=f'{pveFames:,}', inline=True)
                em.add_field(
                    name='PvP Fame', value=f'{pvpFames:,}', inline=True)
                em.add_field(name='\u200b', value='\u200b', inline=True)

                # Fourth row
                em.add_field(
                    name='Gathering Fame',
                    value=f'{gatheringFames:,}',
                    inline=True)
                em.add_field(
                    name='Crafting Fame',
                    value=f'{craftingFames:,}',
                    inline=True)
                em.add_field(name='\u200b', value='\u200b', inline=True)
